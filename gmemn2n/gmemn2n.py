...
The implementation is based on https://arxiv.org/pdf/1610.04211.pdf [1]
"""
...
class GMemN2N(object):
    ...
    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size,
        ...
        session=tf.Session(),
        name='GMemN2N'):
        """Creates an Gated End-To-End Memory Networks
        Args:
            ...
            name: Name of the End-To-End Memory Network. Defaults to `GMemN2N`.
        """
    ...
    def _build_vars(self):
        with tf.variable_scope(self._name):
            ...
            self.W = tf.Variable(self._init([self._embedding_size, self._embedding_size]), name="W")
            self.b = tf.Variable(self._init([1, self._embedding_size]), name="b")
    ...
    def _inference(self, stories, queries):
        with tf.variable_scope(self._name):
            ...
            for hopn in range(self._hops):
                ...
                #Use adaptive gating mechanism to dynamically control the memory reading operation on the controller state.
                T_k = tf.sigmoid(tf.matmul(u[-1], self.W) + self.b)
                u_k = o_k * T_k + u[-1] * (1 - T_k)
